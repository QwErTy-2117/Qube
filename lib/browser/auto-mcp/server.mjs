...
const tools = {
  ...
  snapshot: {
    description: "Capture bounded accessibility snapshot with element refs (e1, e2, … up to 80). Masks password values. Use refs with the act tool. Stale refs self-heal (auto re-match); if recovery fails you get a fresh snapshot back in the error.",
    inputSchema: { type:"object", properties:{ tab_id:{type:"integer"} }, additionalProperties:false },
    fn: async({tab_id})=>{
      let target=null;
      if(tab_id){ const all=await listTargets(); target=all.find(t=> String(t.id)===String(tab_id));}
