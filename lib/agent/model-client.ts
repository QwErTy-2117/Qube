...
// Force stream:true for every AI request — Codex and now any provider per user request
...
export function createModelClient(qualifiedModelId?: string | null): ChatModel {
  ...
  if (provider.id === "chatgpt") {
    throw new Error(
      `ChatGPT model "${modelIdToUse}" requires request context (user session). Use createModelClientForRequest() from a route handler.`
    );
  }
...
export function createModelClientForRequest(
  ...
  if (provider.id === "chatgpt") {
    // Lazy import to avoid circular deps and keep server-only code isolated
    ...
    // ChatGPT proxy speaks Responses API — always stream:true per user request (all providers)
